...
class TestWbExecute(unittest.TestCase):
    ...
    def test_workbench_chain_contents(self):
        o = run("WORKBENCH_ENV_NAME= WORKBENCH_HOME={td}/wbhome/simple "
                ...
                "{wb} r outer/inner/simple1 \$WORKBENCH_CHAIN")
        self.assertEqual(o.stderr, "")
        self.assertEqual(o.returncode, 0)
    ...
    def test_invoke_run_on_bench(self):
        ...
        self.assertEqual(stdout,
            ["ROOT", "OUTER", "INNER", "SIMPLE1", "Default-Run"]
        )
    ...
    def test_workbench_run_override_and_deactivate_on_exit(self):
        """
        ...
        Test this with `wb r` in non-interative manner to verify stdout
        """
        o = run("WORKBENCH_ENV_NAME= "
                "WORKBENCH_RUN_FUNC=exit "
    ...
    def test_disable_realpath_using_env_var(self):
        ...
        self.assertEqual(stdout,
            ["OUTER", "ROOT", "OUTER", "INNER", "SIMPLE1", "Default-Run"]
        )
    ...
    def test_workbench_exec_mode(self):
        """
        WORKBENCH_EXEC_MODE takes the value "a" | "r" | "n"
